...
namespace App\Acf\CustomFields\GradientColorPicker;
 ...
 */
if (!class_exists('App\Acf\CustomFields\GradientColorPicker\\' . FORMATTED_FIELD_NAME)) {
    return;
}
...
class GradientColorPickerField extends acf_field
{
    ...
    */
    ...
    public function render_field($field)
    {
        ...
        $DEFAULT_VALUE = 'linear-gradient(45deg, rgba(109, 136, 255, 1) 0.46153846153846156%,rgba(136, 222, 255, 1) 100%)';
        $value = (isset($field['value']) && !empty($field['value'])) ? $field['value'] : $DEFAULT_VALUE;

        $rgb_values = explode('rgba', $value);
        $rgb_values = [
            'rgba' . explode(')', $rgb_values[1])[0] . ')',
            'rgba' . explode(')', $rgb_values[2])[0] . ')'
        ];

        ?>

        <div class="aky_acf_<?= $this->settings['slug'] ?>" acf_id="<?= $field['key'] ?>">
            <div class="picker-btn" gradient="<?= $rgb_values[0] . '_' . $rgb_values[1]  ?>">
                <span>Choisir un dégradé</span>
                <div class="show-color" style="background : <?= $value ?>;"></div>
            </div>
    ...
    */
    ...
    public function input_admin_enqueue_scripts()
    {
        ...
        add_filter("script_loader_tag", function ($tag, $handle, $src) {
            if ('aky_acf_gradient-color-picker' === $handle) {
